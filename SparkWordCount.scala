...
object WordCount {
  ...
  case class Params(
      ratings: String = "sample_movielens_ratings.txt",
      movies: String = "sample_movielens_movies.txt",
      maxIter: Int = 15,
      regParam: Double = 0.1,
      rank: Int = 10,
  ...
  def main(args: Array[String]) {
   ...
   def run(params: Params) {
    val conf = new SparkConf().setAppName(s"MovieLensALS with $params").setMaster("local").set("spark.executor.memory","1g")
